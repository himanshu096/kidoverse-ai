...
async def start_agent_session(user_id, is_audio=False):
    ...
    try:
        session = await session_service_for_crud.get_session(app_name=APP_NAME, user_id=user_id, session_id="")
...
async def debug_firestore():
    """Debug endpoint to test Firestore connectivity and check current state"""
    try:
        # Test Firestore connectivity
        firestore_ok = test_firestore_connectivity()
        
        # Check current lesson state for a test user
        test_user_id = "test_user_123"
        lesson_state = load_lesson_state_from_firestore(test_user_id)
        
        return {
            "firestore_connectivity": firestore_ok,
            "test_lesson_state": lesson_state,
            "collections": {
                "lesson_collection": "adk_lessons",
                "session_collection": "adk_sessions",
                "completed_lessons_collection": "adk_completed_lessons"
            }
        }
    except Exception as e:
...
async def debug_user_state(user_id: str):
    """Debug endpoint to check lesson state for a specific user"""
    try:
        lesson_state = load_lesson_state_from_firestore(user_id)
        session_state = load_session_state_from_firestore(APP_NAME, user_id)
        
        return {
            "user_id": user_id,
            "lesson_state": lesson_state,
            "session_state_keys": list(session_state.keys()) if session_state else [],
            "session_last_topic": session_state.get("user:last_lesson_topic") if session_state else None
        }
    except Exception as e:
...
@app.get("/debug/test-save/{user_id}")
async def test_firestore_save(user_id: str):
    """Debug endpoint to test Firestore saving directly"""
    try:
        # Create a test lesson state
        test_lesson_state = {
            "current_lesson_plan": {
                "topic": "Test Topic",
                "duration_minutes": 15,
        ...
        # Try to save
        save_result = save_lesson_state_to_firestore(user_id, test_lesson_state)
        
        # Try to read it back
        loaded_state = load_lesson_state_from_firestore(user_id)
        
        return {
            "user_id": user_id,
            "save_result": save_result,
            "loaded_state": loaded_state,
            "test_lesson_state": test_lesson_state
        }
    except Exception as e:
        return {"error": str(e), "user_id": user_id}

@app.get("/debug/session/{user_id}")
async def debug_session_state(user_id: str):
    """Debug endpoint to check current session state for a specific user"""
    try:
        global main_app_runner
        
        if main_app_runner is None:
            return {"error": "Application not initialized"}
        
        session_service = main_app_runner.session_service
        session = await session_service.get_session(app_name=APP_NAME, user_id=user_id, session_id="")
        ...
        return {
            ...
            "session_user_id": session.state.get('user_id'),
            "has_lesson_plan": session.state.get('current_lesson_plan') is not None,
            "lesson_topic": session.state.get('current_lesson_plan', {}).get('topic') if session.state.get('current_lesson_plan') else None,
            "section_index": session.state.get('current_lesson_section_index'),
            "has_markdown": session.state.get('parsed_section_markdowns') is not None,
            "markdown_sections": len(session.state.get('parsed_section_markdowns', []))
        }
    except Exception as e:
        return {"error": str(e), "user_id": user_id}

@app.get("/debug/agent-tools")
async def debug_agent_tools():
    """Debug endpoint to check what tools are available to the root agent"""
    try:
        global root_agent
        ...
        for agent in root_agent.sub_agents:
            ...
            sub_agents_info.append(agent_info)
        
        return {
            "root_agent_name": getattr(root_agent, 'name', 'Unknown'),
            "root_agent_model": getattr(root_agent, 'model', 'Unknown'),
            "tools": tools_info,
            "sub_agents": sub_agents_info,
            "total_tools": len(tools_info),
            "total_sub_agents": len(sub_agents_info)
        }
    except Exception as e:
        return {"error": str(e)}
@app.get("/debug/test-lesson-plan/{user_id}/{topic}")
async def test_lesson_plan_creation(user_id: str, topic: str):
    """Debug endpoint to manually test lesson plan creation and Firestore saving"""
    try:
        global root_agent
        ...
        # Find the plan_lesson_tool
        plan_lesson_tool = None
        for tool in root_agent.tools:
            tool_name = getattr(tool, 'name', None)
            if tool_name == 'plan_lesson_tool':
                plan_lesson_tool = tool
                break
        
        if plan_lesson_tool is None:
            return {"error": "plan_lesson_tool not found"}
        
        print(f"[TEST] Attempting to create lesson plan for topic: {topic}")
        ...
        # Call the lesson planning tool
        try:
            if hasattr(plan_lesson_tool, 'invoke'):
                result = plan_lesson_tool.invoke({"topic": topic}, mock_context)  # type: ignore
            else:
                return {"error": "Tool does not have invoke method"}
        except Exception as invoke_error:
            return {"error": f"Tool invocation failed: {str(invoke_error)}"}
        
        # Safely access the lesson plan from the state after the tool call
        lesson_plan = test_state.get('current_lesson_plan')
        lesson_topic = lesson_plan.get('topic') if isinstance(lesson_plan, dict) else None
        ...
        return {
            ...
            "result": result,
            "final_state": {
                "has_lesson_plan": isinstance(lesson_plan, dict),
                "lesson_topic": lesson_topic,
                "section_index": test_state.get('current_lesson_section_index'),
                "has_markdown": isinstance(test_state.get('parsed_section_markdowns'), list),
                "markdown_sections": len(test_state.get('parsed_section_markdowns', [])) if isinstance(test_state.get('parsed_section_markdowns'), list) else 0
            }
        }
    except Exception as e:
        return {"error": str(e), "user_id": user_id, "topic": topic}
